...
default_config = 'ks/fno_ensemble.ini'
...
def split_list(ls, split_by):
    new_ls = []
    for i, element in enumerate(ls):
        list_number = i // split_by
        if list_number >= len(new_ls):
            new_ls.append([element])
        else:
            new_ls[list_number].append(element)
    return new_ls
...
if __name__ == '__main__':
    ...
    if config['META'].get('only_validate', None):
        ...
        if training_parameters_dict['uncertainty_quantification'][0] == 'ensemble':
            # take 3 checkpoints together
            filename_to_validate = split_list(filename_to_validate, split_by=3)
    ...
    for i, data_parameters in enumerate(data_parameters_dict):
        ...
        for i, training_parameters in enumerate(training_parameters_dict):
            ...
            if training_parameters.get('filename_to_validate', None):
                ...
                if training_parameters['uncertainty_quantification'] == 'ensemble':
                    model = Ensemble(filename, training_parameters, in_channels, out_channels, device)
                else:
